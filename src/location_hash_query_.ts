...
export function location_hash_query_(
	...
	let location_hash_query_a:string[]
	if (decoded_url_hash_str) {
		location_hash_query_a = decoded_url_hash_str.split('&') as string[]
	...
	function decodeURIComponent_location_hash_query() {
		let out_location_hash_query_a = [] as string[]
		for (let i = 0; i < location_hash_query_a.length; i++) {
			out_location_hash_query_a.push(
				decodeURIComponent(location_hash_query_a[i]))
		}
	...
	function reduce(in_location_hash_query_a:string[]) {
